import SetCommModel from "../../models/accounts/setcomm.model.js";
...
class SetCommService {
    ...
    async createSetComm(data) {
        ...
        if (!data.pos_id) throw badRequest("POS ID is required");
        if (!data.ref_id) throw badRequest("Reference ID is required");
        if (!data.insured_name) throw badRequest("Insured Name is required");
    ...
    async getFilterOptions(readScope, reportMonth) {
        const [posOptions, insurers] = await Promise.all([
            SetCommModel.getDistinctPosOptions(readScope, reportMonth),
            SetCommModel.getDistinctInsurers(readScope, reportMonth)
        ]);
    ...
    async getSetCommById(id, readScope) {
        ...
        if (!record) {
            throw notFound("Policy record not found");
        }
        return record;
    }
    ...
    async updateSetComm(id, data, readScope) {
        ...
        if (!existingRecord) {
            throw notFound("Set commission record not found");
        }
        ...
        if (!persisted) {
            throw conflict("Policy commission could not be saved. Please refresh and try again.");
        }
        return {

...
export const validateDocuments = (data) => {
    if (!data.pdfDocument?.buffer) {
        throw badRequest("Policy PDF is required");
    }
    ...
    if ((hasAadhaarFront || hasAadhaarBack) && !hasCompleteAadhaar) {
        throw badRequest("Upload both Aadhaar front and Aadhaar back, or use PAN card/GST certificate instead");
    }

    if (!hasCompleteAadhaar && !hasPanCard && !hasGstCertificate) {
        throw badRequest("Upload at least one KYC document: Aadhaar front & back, PAN card, or GST certificate");
    }
    ...
    if (
        ...
        throw badRequest(`RC front and RC back documents are required for ${businessType}`);
    }
    ...
    if (
        ...
        throw badRequest(`Previous Policy document is required for ${businessType}`);
    }
    ...
    if (
        ...
        throw badRequest(`Invoice document is required for ${businessType}`);
    }
};
...
const validatePolicyData = async data => {
    ...
    if (!String(data.rto || "").trim()) {
        throw badRequest("Registration Authority / RTO is required");
    }

    if (!isPositiveInteger(data.created_by)) {
        throw unauthorized("Authenticated employee is required to save a policy");
    }

    const hierarchyFields = ["bqp_id", "reporting_id", "rm_id", "pos_id", "ref_id"];
    if (!hierarchyFields.every(field => isPositiveInteger(data[field]))) {
        throw badRequest("Please select a valid BQP, reporting manager, relationship manager, POSP and reference");
    }

    if (!(await PoliciesMotorModel.isValidHierarchy(data))) {
        throw badRequest("The selected Motor Entry hierarchy no longer matches. Refresh the dropdowns and select it again.");
    }
};
...
const formatPolicyDates = data => {
    ...
    if (!data.issue_date) {
        throw badRequest("A valid policy issue date is required");
    }
};
...
class PoliciesMotorService {
    ...
    async createPolicy(data) {
        ...
        if (duplicate) {
            throw conflict("Policy Number already exists.");
        }
    ...
    async updatePolicy(id, data, readScope) {
        ...
        if (affectedRows === 0) {
            throw badRequest("No changes made or policy update failed");
        }
    ...
    async deletePolicy(id, readScope) {
        ...
        if (affectedRows === 0) {
            throw notFound("Policy not found or already deleted");
        }
        return {
    ...
    async searchPolicy(keyword, readScope) {
        if (!keyword) {
            throw badRequest("Search keyword required");
        }

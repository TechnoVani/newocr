...
export const resolvePolicyFolderPath = (issueDate, policyNumber) => {
    ...
    if (isNaN(date.getTime())) throw badRequest("Invalid issue date format");

    const cleanPolicyNumber = sanitizePolicyNumber(policyNumber);
    if (!cleanPolicyNumber) {
        throw badRequest("Policy number does not contain any valid folder characters");
    }
...
class FolderService {
    ...
    async createPolicyFolder(issueDate, policyNumber) {
        if (!issueDate) {
            throw badRequest("Issue date is required to create policy folder");
        }
        if (!policyNumber) {
            throw badRequest("Policy number is required to create policy folder");
        }
    ...
    async removeNewPolicyFolder(folderPath) {
        ...
        if (
            ...
            throw forbidden("Unable to remove this policy folder.");
        }

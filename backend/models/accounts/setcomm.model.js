...
const getMonthRange = reportMonth => {
...
class SetCommModel {
    ...
    static async findById(id, readScope) {
        ...
        const query = `SELECT * FROM policies_motor WHERE id = ? AND ${ownership.sql}`;
    ...
    static async findAll(options = {}) {
        ...
        let query = `SELECT ${POLICY_REPORT_SELECT} FROM policies_motor p ${POLICY_REPORT_JOINS}`;
        let countQuery = `SELECT COUNT(*) as count FROM policies_motor p ${POLICY_REPORT_JOINS}`;
    ...
    static async update(id, data, readScope) {
        ...
        const ownership = policyOwnershipFilter(readScope, "created_by");
        const query = `UPDATE policies_motor SET ${updateQuery} WHERE id = ? AND ${ownership.sql}`;
    ...
    static async delete(id, readScope) {
        const ownership = policyOwnershipFilter(readScope, "created_by");
        const query = `DELETE FROM policies_motor WHERE id = ? AND ${ownership.sql}`;
     ...
     */
    static async getDistinctPosOptions(readScope, reportMonth) {
        const ownership = policyOwnershipFilter(readScope, "p.created_by");
        const monthRange = getMonthRange(reportMonth);
        const monthFilter = monthRange
            ? " AND p.issue_date >= ? AND p.issue_date < ?"
            : "";
        const params = monthRange
            ? [...ownership.params, monthRange.startDate, monthRange.endDate]
            : ownership.params;
        const query = `
            SELECT DISTINCT
                p.pos_id AS value,
     ...
     */
    static async getDistinctInsurers(readScope, reportMonth) {
        const ownership = policyOwnershipFilter(readScope, "p.created_by");
        const monthRange = getMonthRange(reportMonth);
        const monthFilter = monthRange
            ? " AND p.issue_date >= ? AND p.issue_date < ?"

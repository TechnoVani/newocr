...
class SetCountModel {
     ...
     * @param {object} data - Set Count fields
    ...
    /**
     * Find a Set Count record by ID
     * @param {number} id - Record ID
     * @returns {Promise<object|null>} Record or null
     */
    static async findById(id, userId) {
        const query = "SELECT * FROM policies WHERE id = ? AND created_by = ?";
        const [rows] = await db.query(query, [id, userId]);
        return rows[0] || null;
    }

    /**
     * Find all Set Count records with optional filtering, sorting, and pagination
    ...
    static async findAll(options = {}) {
        ...
        let queryParams = [options.userId];
        let whereClauses = ["p.created_by = ?"];
    ...
    static async update(id, data, userId) {
        ...
        const query = `UPDATE policies SET ${updateQuery} WHERE id = ? AND created_by = ?`;
    ...
    /**
     * Delete a Set Count record by ID
     * @param {number} id - Record ID
     * @returns {Promise<number>} Affected rows count
     */
    static async delete(id, userId) {
        const query = "DELETE FROM policies WHERE id = ? AND created_by = ?";
     ...
     */
    static async getDistinctPosIds(userId) {
        const query = `
            SELECT DISTINCT pos_id 
            FROM policies
            WHERE created_by = ? AND pos_id IS NOT NULL AND pos_id != ''
            ORDER BY pos_id ASC
        `;
        const [rows] = await db.query(query, [userId]);
        return rows.map(r => r.pos_id);
    }

    /**
     * Get distinct Insurance Companies logged in set_count
     */
    static async getDistinctInsurers(userId) {
        const query = `
            SELECT DISTINCT insurance_company 
            FROM policies
            WHERE created_by = ? AND insurance_company IS NOT NULL AND insurance_company != ''
            ORDER BY insurance_company ASC
        `;
        const [rows] = await db.query(query, [userId]);

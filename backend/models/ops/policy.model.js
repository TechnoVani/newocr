...
class PolicyModel {
    ...
    static async findByRefId(refId, readScope) {
        const ownership = policyOwnershipFilter(readScope, "created_by");
        const query = `SELECT * FROM policies WHERE ${ownership.sql} AND (ref_id = ? OR policy_number = ?) LIMIT 1`;
    ...
    static async getAllReferenceIds(readScope) {
        const ownership = policyOwnershipFilter(readScope, "created_by");
        const query = `SELECT DISTINCT ref_id, policy_number, insured_name, created_by FROM policies WHERE ${ownership.sql} AND ref_id IS NOT NULL AND ref_id != '' ORDER BY ref_id ASC`;
        const [rows] = await db.query(query, ownership.params);
        return rows;
    }
    ...
    static async findMonthlyReport(startDate, endDate, readScope) {
        ...
        `;
        const [rows] = await db.query(query, [...ownership.params, startDate, endDate]);
        return rows;
    }
    ...
    static async userOwnsPolicyFolder(readScope, folderName) {
        const ownership = policyOwnershipFilter(readScope, "created_by");
        const query = `SELECT policy_number FROM policies WHERE ${ownership.sql}`;
        const [rows] = await db.query(query, ownership.params);

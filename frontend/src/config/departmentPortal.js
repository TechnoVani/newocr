...
const BY_NAME = new Map([
  ...
  ["risk management", "risk-management"], ["crm (customer relationship management)", "crm"],
  ["crm", "crm"],
]);
...
export const isAdmin = (user) => isAdministration(user) || ["admin", "administrator"].includes(normalize(user?.user_type));
...
export const getDefaultPortalPath = (user) => {
  const ownDepartmentPortal = BY_NAME.get(normalize(user?.department));
  if (ownDepartmentPortal) return `/${ownDepartmentPortal}`;

...
export default function DependentInsuranceForm({ title, fields = [], onSubmit }) {
  const initial = useMemo(() => Object.fromEntries(fields.map(({ name, defaultValue }) => [name, defaultValue || ""])), [fields]);
        ...
        return (
          <label key={field.name} className={formLabelClass}>
            ...
            {field.type === "select" ? (
              ...
              <textarea required={field.required} value={values[field.name] || ""} onChange={(event) => update(field, event.target.value)} className={`${formControlClass} min-h-24 py-3`} placeholder={`Enter ${field.label.toLowerCase()}`} />
            ) : (

      ...
      <div className="flex-1 overflow-x-auto [-webkit-overflow-scrolling:touch]">
        <table className="w-max min-w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr>
                <td colSpan={columns.length + (showSerialNumber ? 1 : 0)} className="py-10 text-center text-xs font-semibold text-slate-400">{loadingMessage}</td>
              </tr>
            ) : visible.length ? visible.map((row, index) => (
              <tr
                key={row.report_row_id ?? row.id ?? index}
                className={rowClassName
                  ? `${rowClassName(row, index)} transition-colors`
                  : `${index % 2 === 0 ? "bg-white" : "bg-blue-50/40"} transition-colors hover:bg-blue-100/50`}
              >
                {showSerialNumber && (
              ...
              <tr>
                <td colSpan={columns.length + (showSerialNumber ? 1 : 0)} className="bg-slate-50/5 py-10 text-center text-xs font-semibold italic text-slate-400">{emptyMessage}</td>
              </tr>
            )}
          </tbody>
        </table>

...
const InfoField = ({ label, name, value, type = "text", icon: Icon, options = null, isEditing, formData, handleInputChange, formatDate }) => {
  ...
  return (
    <div className="relative mt-2 flex flex-col rounded-xl border border-slate-200/80 bg-white p-3 transition-all hover:bg-slate-50/50">
      <label className="pointer-events-none absolute -top-1.5 left-3 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">
      ...
      <div className="flex items-center gap-2 min-h-[20px]">
        ...
        {options ? (
          <ReusableSelect
            ...
            className="cursor-pointer bg-slate-50/50"
          >
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    axiosInstance.get("/auth/profile/picture", { responseType: "blob" })
      .then((response) => {
        if (!active) return;
        if (response.status === 204 || !response.data?.size) {
          ...
          <form onSubmit={handleAccountSubmit} className="mt-5 rounded-2xl border border-blue-100 bg-blue-50/40 p-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">Account holder name <span className="text-red-500">*</span></span>
                <input name="account_holder_name" value={accountForm.account_holder_name} onChange={handleAccountInputChange} maxLength={255} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs text-slate-800 outline-none focus:border-blue-400" />
              </label>
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">Bank name <span className="text-red-500">*</span></span>
                <input name="bank_name" value={accountForm.bank_name} onChange={handleAccountInputChange} maxLength={255} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs text-slate-800 outline-none focus:border-blue-400" />
              </label>
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">Account number <span className="text-red-500">*</span></span>
                <input name="account_number" value={accountForm.account_number} onChange={handleAccountInputChange} inputMode="numeric" autoComplete="off" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-mono text-xs text-slate-800 outline-none focus:border-blue-400" />
              </label>
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">IFSC code <span className="text-red-500">*</span></span>
                <input name="ifsc_code" value={accountForm.ifsc_code} onChange={handleAccountInputChange} maxLength={11} placeholder="SBIN0001234" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-mono text-xs uppercase text-slate-800 outline-none focus:border-blue-400" />
              </label>
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">Branch name</span>
                <input name="branch_name" value={accountForm.branch_name} onChange={handleAccountInputChange} maxLength={255} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs text-slate-800 outline-none focus:border-blue-400" />
              </label>
              <label className="relative block pt-2">
                <span className="pointer-events-none absolute left-3 top-0 z-10 bg-white px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">Account type</span>
                <ReusableSelect name="account_type" value={accountForm.account_type} onChange={handleAccountInputChange}>
              ...
              return (
                <div key={name} className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
                  ...
                  <div className="relative mt-3 pt-2">
                    <label htmlFor={`employee-document-${name}`} className="pointer-events-none absolute left-3 top-0 z-10 max-w-[calc(100%-1.5rem)] truncate bg-slate-50 px-1 text-[9px] font-bold uppercase leading-none tracking-wider text-slate-500">{label}</label>
                    <input
                      id={`employee-document-${name}`}
                      type="file"
                      accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png"
                      onChange={(event) => handleDocumentChange(event, name)}
                      className="block w-full rounded-xl border border-slate-200 bg-white px-2 py-2 text-[11px] text-slate-500 file:mr-2 file:rounded-lg file:border-0 file:bg-blue-50 file:px-3 file:py-2 file:text-[11px] file:font-semibold file:text-blue-700 hover:file:bg-blue-100"
                    />
                  </div>

...
// import { submitMotorPolicy } from '../redux/actions/motorActions';
// import { resetMotorState } from '../redux/slices/motorSlice';
// import { DROPDOWN_STEPS, getCustomStyles } from './MotorEntrySection';
...
//       }
//     });
//   }, [initialPolicyNumber, initialInsuranceCompany, initialBranchAddress, initialProductType, initialVehicleCategory, initialInsuredName, initialPanNumber, initialGstin, initialContactNumber, initialEmail, initialInsuredAddress, initialPolicyDates, initialDateOfIssue, initialTotalValue, initialPreviousInsurer, initialPreviousPolicyNumber, initialFinalPremium, initialVehicle]);

//   const displayFileName = item?.fileName || "Unknown file";
...
//       toast.error("If Aadhaar is uploaded, both front and back files are required.");
//       return;
//     }
//     if (!hasPanCard && !(hasAadhaarFront && hasAadhaarBack)) {
//       toast.error("Upload either a PAN card or both Aadhaar front and back files.");
//       return;
//     }
...
//       const result = await dispatch(submitMotorPolicy(rawFile, payload));
//       if (result.success) {
//         if (onSubmit) onSubmit(result.data);
//         resolvedMotorProps.onSubmitSuccess?.(result.data);
//         dispatch(resetMotorState());
//       }
//     } catch (error) {
...
//     fuelType: formData.vehicle.fuelType || extractedVehicle?.fuelType,
...
//   // ----- Render -----
//   return (
//     <div className="w-full rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.03)] border border-slate-100 bg-white overflow-hidden transition-all duration-300 hover:shadow-[0_15px_45px_rgb(0,0,0,0.05)]">
//       <style>{`
...
//                 KYC required: upload a PAN card, or upload both Aadhaar front and Aadhaar back.
//               </div>
//               <EditableRow label="Aadhaar Front (Pair)" value={mergedVehicle.aadhaarFront} onChange={(file) => handleVehicleChange("aadhaarFront", file)} type="file" />
...
function PolicyCardView({
  ...
  }, [initialPolicyNumber, initialInsuranceCompany, initialBranchAddress, initialProductType, initialVehicleCategory, initialInsuredName, initialPanNumber, initialGstin, initialContactNumber, initialEmail, initialInsuredAddress, initialPolicyDates, initialDateOfIssue, initialTotalValue, initialPreviousInsurer, initialPreviousPolicyNumber, initialFinalPremium, initialVehicle, extractedVehicle]);

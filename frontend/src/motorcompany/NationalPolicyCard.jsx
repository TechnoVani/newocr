...
// panNumber remains "-" if no valid PAN found
  ...
  let contactMatch = normalizedText.match(/Contact\s+Number\s*[:：]\s*([\d]+)/i);
  if (!contactMatch) contactMatch = normalizedText.match(/संपकर\s*संखया\s*\/\s*Contact\s+Number\s*[:：]\s*([\d]+)/i);
  if (!contactMatch) contactMatch = normalizedText.match(/सेल\s*\/\s*Cell\s*[:：]\s*([\d]+)/i);
  if (!contactMatch) contactMatch = normalizedText.match(/फोन\s*Phone\s*[:：]\s*([\d]+)/i);
  if (!contactMatch) contactMatch = normalizedText.match(/Telephone\s*[:：]\s*([X\d]+)/i);
  ...
  const gstinMatch = normalizedText.match(/GSTIN\s*[:：]?\s*([A-Z0-9]{15})/i) ||
                     normalizedText.match(/जीएसटीआईएन\s*\/\s*GSTIN\s*[:：]?\s*([A-Z0-9]{15})/i);

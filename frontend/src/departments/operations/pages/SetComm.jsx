...
export default function SetComm() {
  ...
  useEffect(() => {
    // Remote commission synchronization intentionally updates state after mount/filter changes.
      ...
      return {
        ...
        render: (_, policy) => {
          ...
          if (column.key === "policy_status") {
            const cancelled = String(merged.policy_status || "").toLowerCase() === "cancelled";
            return (
              <span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase ${cancelled ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>
                {merged.policy_status || "Active"}
              </span>
            );
          }
          if (isCommission) {
            // ✅ Applied % symbol right end for ALL commission input fields
            return (
              <span className="relative inline-flex items-center">
                <input
                  type="number"

...
const extractInsuredDetails = (text = "") => {
  ...
  // ============================================================
  let panNumber = "-";
  const panMatch = normalizedText.match(/PAN\s+Number\s*:?\s*([A-Z0-9]+)/i);
  if (panMatch && panMatch[1]) {
    panNumber = panMatch[1].toUpperCase();
  }
  // GSTIN is not typically the insured's, so we leave as "-"
...
const extractVehicleDetailsFromText = (text = "") => {
  ...
  if (commercialMatch) result.commercialVehicleType = commercialMatch[1].trim();

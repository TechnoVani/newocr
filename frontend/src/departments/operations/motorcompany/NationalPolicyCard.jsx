...
const extractPremiumData = (text) => {
  const result = { totalOdPremium: "-", totalTpPremium: "-", netPremium: "-", gst: "-", totalPayable: "-", calculatedOdPremium: "-", calculatedTpPremium: "-" };
  const cleanNumber = (value) => value ? value.replace(/,/g, "").trim() : "-";
...
const extractVehicleDetailsFromText = (text) => {
  const result = {
    registrationNumber: "-", chassisNumber: "-", engineNumber: "-", make: "-", model: "-", variant: "-",
    manufacturingYear: "-", cubicCapacity: "-", seatingCapacity: "-", geographicalArea: "-", financierName: "-",
    regDistrict: "-", fuelType: "-", ncb: "0%"
  };
  ...
  if (isCommercial) {
    ...
    // --- CLEANED EXTRACTION BLOCKS (REMOVED DESTRUCTIVE .replace() calls) ---

    const makeMatch = text.match(
      /Make\s*[:：]?\s*([\s\S]*?)(?=\s*(?:कुल\s*कीमत|Total\s+Value|मॉडल|Model|अित|A\s+ddl|वेिरएंट|Variant|सी\s*\.\s*सी|CC\s*\/\s*GVW|वाहन\s*की\s*शेणी|Class\s+of\s+Vehicle|ईधन\s*का\s*पकार|Type\s+of\s+Fuel|लाइसेस|Licensed|ढाचा|Body\s+Type|िनमारण|Year\s+of\s+Mfg|$))/i
    ) || text.match(/बनावट\s*Make\s*[:：]?\s*([\s\S]*?)(?=\s*(?:कुल\s*कीमत|Total\s+Value|मॉडल|Model|अित|A\s+ddl|वेिरएंट|Variant|सी\s*\.\s*सी|CC\s*\/\s*GVW|वाहन\s*की\s*शेणी|Class\s+of\s+Vehicle|ईधन\s*का\s*पकार|Type\s+of\s+Fuel|लाइसेस|Licensed|ढाचा|Body\s+Type|िनमारण|Year\s+of\s+Mfg|$))/i);
    if (makeMatch) {
      result.make = makeMatch[1].replace(/\n/g, " ").replace(/\s+/g, " ").trim();
    }
    ...
    if (modelMatch) {
      result.model = modelMatch[1].replace(/\n/g, " ").replace(/\s+/g, " ").trim();
    }
    ...
    if (variantMatch) {
      result.variant = variantMatch[1].replace(/\n/g, " ").replace(/\s+/g, " ").trim();
    }
    ...
    if (makeModel) {
      ...
      if (tokens.length > 2) {
        const stopWords = /^(|Stop|\d)$/i;
        result.variant = tokens.slice(2).filter(t => !stopWords.test(t)).join(" ").trim();
      } else {
        result.variant = "-";
      }
    }

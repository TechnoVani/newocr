...
const extractVehicleDetails = (fullText = "") => {
  ...
  // Assuming normalizeText and removeHyphens are defined elsewhere in your code
  // If not, replace these with basic string methods like .toUpperCase()
  ...
  if (regMatch) {
    ...
    // Replace removeHyphens with reg.replace(/-/g, "") if the helper function is missing
    result.registrationNumber = reg.replace(/-/g, "") || "-"; 
  }

  // 2. Engine & Chassis Number
  // Looks for two 10+ character alphanumeric strings separated by an '&'
  ...
  // To prevent matching the table headers, we scope the next search to 
  // only look at the text *after* the chassis number we just found.
  ...
  // 3. Make / Model / Variant
  // Captures Make (before hyphen), Model, and Variant (e.g., BS 6) before the Body Type
  const makeModelMatch = postChassisText.match(/^\s*([A-Z\s]+?)\s*-\s*(.*?)\s+(BS\s*[A-Z0-9]+)\s+(?=[A-Z\s]+\/)/i);
  
  if (makeModelMatch) {
    result.make = makeModelMatch[1].replace(/\s+/g, " ").trim();     // MARUTI SUZUKI
    result.model = makeModelMatch[2].replace(/\s+/g, " ").trim();    // DZIRE VXI CNG
    result.variant = makeModelMatch[3].replace(/\s+/g, " ").trim();  // BS 6
  } else {
    // Fallback just in case "BS 6" or similar variant string isn't present
    const fallbackMatch = postChassisText.match(/^\s*([A-Z\s]+?)\s*-\s*([A-Z0-9\s]+?)\s+(?=[A-Z\s]+\/)/i);
    if (fallbackMatch) {
...
function ShriramPolicyCard({ item }) {
  ...
  const mergedVehicle = {
    ...
    commercialVehicleType: vehicle?.commercialVehicleType || vehicleDetails?.commercialVehicleType || "-",
    subType: vehicle?.subType || vehicleDetails?.subType || "-",
  };

...
import { BadgeCheck, Building2, ChevronDown, ContactRound, FileSpreadsheet, FileText, FileX2, GitBranch, Home, Menu, SearchCheck, Settings2, ShieldCheck, Upload, Users, X } from "lucide-react";
...
const reportLinks = [
  ...
  { path: "/accounts/reports/pos-wise", label: "POS Wise", icon: ContactRound },
  { path: "/accounts/reports/cancelled-policies", label: "Cancelled Policy", icon: FileX2 },
];
...
const payoutGridLinks = [
  { path: "/accounts/payout-grid/upload", label: "Upload Excel", icon: Upload },
  { path: "/accounts/payout-grid/report", label: "Payout Report", icon: SearchCheck },
];
...
export default function AccountsNavbar() {
  ...
  const { user } = useAuth();
  const canManageAccounts = hasMinimumRole(user, ACCESS_ROLES.MANAGER);
  const showSetCommission = canAccessSetCommission(user);
  const visiblePayoutGridLinks = canManageAccounts ? payoutGridLinks : payoutGridLinks.filter(({ path }) => !path.endsWith("/upload"));
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-sm">
      <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-4 lg:px-12">
        <NavLink to="/accounts" className="flex shrink-0 items-center" aria-label="Accounts dashboard">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          ...
          {showSetCommission && <NavLink to="/set-comm" className={navLinkClass}><Settings2 size={17} aria-hidden="true" />Set Commission</NavLink>}
      ...
      {mobileOpen && <nav className="absolute z-40 flex w-full flex-col gap-2 border-b border-gray-200 bg-white px-4 py-4 shadow-lg lg:hidden">
        ...
        {showSetCommission && <NavLink to="/set-comm" onClick={() => setMobileOpen(false)} className={navLinkClass}><Settings2 size={18} aria-hidden="true" />Set Commission</NavLink>}

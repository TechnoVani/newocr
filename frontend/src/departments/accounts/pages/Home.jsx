    ...
    <main className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col px-2.5 py-4 sm:px-4 lg:px-6">
          ...
          <article className="rounded-xl border border-slate-200/80 bg-slate-900 p-4 text-white shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-2xl font-black">{payout.rules.toLocaleString("en-IN")} rules</p>
                <p className="mt-1 text-xs text-slate-400">{payout.batches} upload batches · {payout.companies} insurers</p>
              </div>
            ...
            <div className="mt-4 flex items-center justify-between rounded-lg bg-white/5 px-3 py-2.5">
              ...
              <Link to="/accounts/payout-grid/report" className="inline-flex items-center gap-1 text-xs font-bold text-blue-300 hover:text-white">Open report <ArrowRight size={13} /></Link>
            </div>
          </article>
      ...
      {(policies.pending_verification > 0 || policies.pending_payment > 0) && (
        <section className="mt-4 flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link to="/accounts/reports/verify" className="inline-flex shrink-0 items-center gap-2 text-xs font-black text-amber-800 hover:text-amber-950">Resolve queue <ArrowRight size={14} /></Link>
        </section>
      )}
    </main>
  );

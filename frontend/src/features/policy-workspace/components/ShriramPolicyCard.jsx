...
const sumNumbers = (...values) =>
  values.reduce((total, value) => {
    const number = parseFloat(cleanAmount(value));
...
const extractIDV = (fullText = "") => {
  ...
  const idvTableMatch = text.match(
    /IDV\s*FOR\s*THE\s*VEHICLE[\s\S]*?TOTAL\s*VALUE\s*([\s\S]*?)(?=Own\s*Damage\s*Policy\s*Period|Liability\s*Policy\s*Period|$)/i
  );
  if (idvTableMatch?.[1]) {
    const amounts = [...idvTableMatch[1].matchAll(/(\d+)\.00/g)]
...
const extractVehicleDetails = (fullText = "") => {
  ...
  const makeModelMatch = postChassisText.match(/^\s*([A-Z\s]+?)\s*-\s*([A-Z0-9]+)\s+([A-Z0-9\s]*?(?:OBD2B|BS\s*[A-Z0-9]+|STD|DLX|DISC|DRUM|SELF|CAST|ALLOY)[A-Z0-9\s]*?)\s+(?=[A-Z\s]+\/\s*[A-Z]+\s*\d+\s*\/\s*\d+(?:\.\d+)?\s*\/\s*\d{4})/i);
  ...
  const specMatch = text.match(
    /(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)\s*\/\s*(\d{4})\s*(\d{2}\/\d{2}\/\d{4})\s*(\d+\s*\+\s*\d+)/i
  );
  if (specMatch) {
    result.cubicCapacity = String(Math.round(parseFloat(specMatch[1])) || specMatch[1]).trim();

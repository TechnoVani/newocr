...
//                          text.match(/Total OD Premium in\s*([\d.]+)/i) || 
...
//   const ncbMatch = text.match(/(?:Total|Calculated)\s+NCB\s+Discount[^\d]*(\d+)\s*%/i) || 
//                    text.match(/No\s+Claim\s+Bonus\s*[:]?\s*[-]?\s*(\d+%)/i);

//   if (ncbMatch?.[1]) {
//     let ncbValue = ncbMatch[1].trim();
//     if (!ncbValue.includes('%')) {
//       ncbValue = `${ncbValue}%`;
//     }
//     result.ncb = ncbValue;
//   }
...
const extractBranchAddress = (fullText = "") => {
  ...
  let match = fullText.match(/POLICY ISSUING OFFICE:\s*([\s\S]*?)(?=\s*(?:PHONE NUMBER|FAX NUMBER|Email|BUSINESS CHANNEL|CLAIM CONTACT|INSURED DETAILS|\n\s*\n))/i);
  if (match) {
    let addressBlock = match[1].trim();
    addressBlock = addressBlock.replace(/\s*PHONE NUMBER:.*$/i, '');
    addressBlock = addressBlock.replace(/\s*FAX NUMBER:.*$/i, '');
...
const extractInsuredDetails = (text = "") => {
  ...
  const panMatch = normalizedText.match(/PAN No\s*[:]?\s*([A-Z0-9]+)/i);
  const panNumber = panMatch?.[1] && !/^NA$/i.test(panMatch[1]) ? panMatch[1] : "-";
  
  const contactMatch = normalizedText.match(/Contact Number\s*\/\s*[\/\s]*([X\d]+)/i);
  const contactNumber = contactMatch?.[1] || "-";

  const emailMatch = normalizedText.match(/Email\s+([^\s]+@[^\s]+(?:\s+[A-Z]{2,})?)/i);
  const email = emailMatch?.[1]
    ? emailMatch[1].replace(/\s+/g, "").replace(/\.(?=$)/, ".com")
    : "-";
...
const extractVehicleDetailsFromText = (text = "") => {
  ...
  // ============================================================
  // Registration number – keep hyphens removed
  // ============================================================
    let registrationNumber = "-";

    const vehicleRegPattern = /([A-Z]{2}[-\s]?\d{1,2}[A-Z]?[-\s]?[A-Z]{1,3}[-\s]?\d{3,4})/i;
    let regMatch = normalizedText.match(
      new RegExp(`VEHICLE\\s+DETAILS[\\s\\S]*?Registration\\s*(?:no\\.?|Number)\\s*[:\\-]?\\s*${vehicleRegPattern.source}`, "i")
    );
    if (!regMatch) {
      regMatch = normalizedText.match(
        new RegExp(`Registration\\s*(?:no\\.?|Number)\\s*[:\\-]?\\s*${vehicleRegPattern.source}`, "i")
      );
    }
    if (regMatch?.[1]) {
      registrationNumber = regMatch[1].trim();
      ...
      const fallbackMatch = normalizedText.match(
        new RegExp(`(?:Reg(?:istration)?\\s*no\\.?|Registration\\s*Number)\\s*[:\\-]?\\s*${vehicleRegPattern.source}`, "i")
      );
      if (fallbackMatch?.[1]) registrationNumber = fallbackMatch[1].trim();
    }
  ...
  // ============================================================
  // FIXED: Chassis and Engine extraction (Prevents catastrophic trailing matches)
  // ============================================================
  
  // Attempt 1: Stop exactly at known boundaries. 
  // Perfectly captures engine numbers containing slashes and spaces like "4H.3 205/2020248"
  const strictCeMatch = normalizedText.match(/Chassis\s*no\.?\s*\/\s*Engine\s*(?:no\.?|Number)\s*[:\-]?\s*([A-Z0-9~]+)\s*\/\s*([A-Z0-9.\-\/\s]+?)(?=\s*(?:Type\s+of\s+fuel|Make|Year|Gross|Cubic|Seating|Variant|Model|$))/i);

  if (strictCeMatch) {
    result.chassisNumber = strictCeMatch[1].replace(/[^A-Z0-9~]/gi, "").toUpperCase();
    result.engineNumber = strictCeMatch[2].replace(/\s+/g, "").trim().toUpperCase();
  } else {
    // Attempt 2: Bounded length capture.
    const boundedCeMatch = normalizedText.match(/Chassis\s*no\.?\s*\/\s*Engine\s*(?:no\.?|Number)\s*[:\-]?\s*([A-Z0-9~]+)\s*\/\s*([A-Z0-9.\-\/ ]{5,35})/i);
    if (boundedCeMatch) {
      result.chassisNumber = boundedCeMatch[1].replace(/[^A-Z0-9~]/gi, "").toUpperCase();
      result.engineNumber = boundedCeMatch[2].replace(/\s+/g, "").trim().toUpperCase();
    }
  }

  // Fallback ONLY if the strict matches completely missed
  if (result.chassisNumber === "-" || result.engineNumber === "-") {
    const legacyChassis = formatChassisNumber("", normalizedText);
    const legacyEngine = formatEngineNumber("", normalizedText);
    if (legacyChassis !== "-") result.chassisNumber = legacyChassis;
    if (legacyEngine !== "-") result.engineNumber = legacyEngine;
  }
  // ============================================================
  ...
  if (bodyFuelCombined?.[1]) {
    ...
    const parts = combinedText.split("/");
    if (parts.length >= 2) {
      result.fuelType = parts[parts.length - 1].trim().toUpperCase();
    } else {
      result.fuelType = combinedText.trim().toUpperCase();
    }
  }
  if (result.fuelType === "-") {
    let fuelMatch = normalizedText.match(/Type of fuel\s*:\s*([^\n]+?)(?=\s*Cubic|$)/i);
  ...
  const ccMatch = normalizedText.match(/Cubic capacity\(cc\).*?(\d{2,5}\s*cc)/is);
  if (ccMatch?.[1]) result.cubicCapacity = ccMatch[1].replace(/\D/g, "").trim();

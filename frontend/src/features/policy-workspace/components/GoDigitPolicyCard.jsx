...
//   const result = {
//     calculatedOdPremium: "-", // Logic removed, defaults to "-"
//     calculatedTpPremium: "-", // Logic removed, defaults to "-"
//     totalOdPremium: "-",
//     totalTpPremium: "-",
//     netPremium: "-",
//     gst: "-",
...
//       result.gst = (cgst + sgst).toFixed(2);
//     }
//   } else {
...
const extractPremiumData = (text = "") => {
  ...
  if (result.totalPayable === "-") {
    // NEW: Logic for totalPayable row ending in 11883.57
    const rowMatch = normalizedText.match(
      /Legal\s+Liability\s+to\s+Paid\s+Driver[\s\S]*?[\d.]+\s+[\d.]+\s+[\d.]+\s+([\d.]+)/i
    );
    if (rowMatch && rowMatch[1]) {
       result.totalPayable = rowMatch[1].replace(/,/g, '');
    }
...
const extractVehicleDetailsFromText = (text = "") => {
  ...
  // Inside extractVehicleDetailsFromText function:

  // 1. Try matching the standard "No Claim Bonus" label
  let ncbMatch = text.match(/No\s+Claim\s+Bonus\s*[:]?\s*[-]?\s*(\d+%)/i);

  if (ncbMatch) {
    result.ncb = ncbMatch[1];
  } else {
    // 2. Fallback: Try matching "NCB % (Current Policy) 35%" format
    // This regex allows for optional "(Current Policy)" text
    ncbMatch = text.match(/NCB\s*%\s*(?:\(Current\s+Policy\))?\s*(\d+%)/i);
    if (ncbMatch) {
      result.ncb = ncbMatch[1];
    }
  }

...
const extractBranchAddress = (fullText) => {
  ...
  const servicingMatch = normalized.match(
    /Policy\s+Servicing\s+office\s*:\s*([\s\S]*?)(?=\s*PH:|PolicyRef\s+No\.?|Period\s+of\s+Insurance|$)/i
  );
  if (servicingMatch?.[1]) {
    return cleanAddress(servicingMatch[1].replace(/\s+/g, " ").trim());
  }
...
const extractInsuredDetails = (text) => {
  ...
  const gstinValue = gstMatch?.[1]?.trim() || "-";
  const gstin = /^[0-9A-Z]{15}$/i.test(gstinValue) ? gstinValue : "-";
...
const extractPremiumData = (text) => {
  ...
  const isLiabilityOnlyPolicy = /(?:TWO\s+WHEELER|PRIVATE\s+CAR)[\s\S]{0,80}(?:LIABILITY\s+ONLY|TP\s+ONLY)/i.test(normalized);
  if (isLiabilityOnlyPolicy) {
    result.totalOdPremium = "0";
    if (result.netPremium === "-" && result.totalTpPremium !== "-") {
...
const extractVehicleDetails = (text) => {
  ...
  const motorVehicleTableMatch = normalized.match(
    /INSURED\s+MOTOR\s+VEHICLE\s+DETAILS\s+AND\s+PREMIUM\s+COMPUTATION[\s\S]{0,600}?Licensed\s+Carrying\s+capacity\s+including\s+Driver\s+([A-Z]{2}\d{1,2}[A-Z]{1,3}\d{4})\s+(\d{4})\/\d{2}-\d{2}-\d{4}\/-\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+NA\s+NA\s+([A-Z][A-Z\s]+?)\/([A-Z0-9\s]+?)\/([A-Z0-9\s-]+?)\s+Solo\s+with\s+Pillion\s+Solo\s+with\s+Pillion\s+NA\s+(\d{2,5})\s+(\d{1,3})\b/i
  ) || normalized.match(
    /INSURED\s+MOTOR\s+VEHICLE\s+DETAILS\s+AND\s+PREMIUM\s+COMPUTATION[\s\S]{0,700}?Licensed\s+Carrying\s+capacity\s+including\s+Driver\s+([A-Z]{2}\d{1,2}[A-Z]{1,3}\d{4})\s+(\d{4})\/\d{2}-\d{2}-\d{4}\/-\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+NA\s+NA\s+([A-Z][A-Z\s]+?)\/([A-Z\s]+?)\/([A-Z0-9\s-]+?)\s+Solo\s+with\s+Pillion\s+Solo\s+with\s+Pillion\s+NA\s+(\d{2,5})\s+(\d{1,3})\b/i
  );

  if (motorVehicleTableMatch) {
    result.registrationNumber = removeHyphens(motorVehicleTableMatch[1]);
    result.manufacturingYear = motorVehicleTableMatch[2];
    const splitEngineChassis = motorVehicleTableMatch.length >= 12;
    result.engineNumber = splitEngineChassis
      ? `${motorVehicleTableMatch[3]}${motorVehicleTableMatch[4]}`.toUpperCase()
      : motorVehicleTableMatch[3].toUpperCase();
    result.chassisNumber = splitEngineChassis
      ? `${motorVehicleTableMatch[5]}${motorVehicleTableMatch[6]}`.toUpperCase()
      : motorVehicleTableMatch[5].toUpperCase();
    const makeIndex = splitEngineChassis ? 7 : 6;
    result.make = cleanValue(motorVehicleTableMatch[makeIndex]);
    result.model = cleanValue(motorVehicleTableMatch[makeIndex + 1])
      .replace(/\bPASSI\s+ON\b/i, "PASSION")
      .replace(/\s+/g, " ")
      .trim();
    result.variant = cleanValue(motorVehicleTableMatch[makeIndex + 2])
      .replace(/-\s*(?:Hatch\s*Back|Sedan|Saloon|SUV|MUV|Jeep|Van)\s*$/i, "")
      .replace(/\s+(?:Hatch\s*Back|Sedan|Saloon|SUV|MUV|Jeep|Van)\s*$/i, "")
      .replace(/-\s*$/, "")
      .trim();
    result.cubicCapacity = motorVehicleTableMatch[makeIndex + 3];
    result.seatingCapacity = motorVehicleTableMatch[makeIndex + 4];
  }

  // ----- Registration Number (unchanged, works) -----
  let regMatch = result.registrationNumber === "-" ? normalized.match(/Vehicle Registration No\.\s*([A-Z0-9]+)/i) : null;
  if (!regMatch && result.registrationNumber === "-") {
  ...
  if (regMatch && regMatch[1]) {
    const candidateReg = removeHyphens(regMatch[1].trim());
    if (/^[A-Z]{2}\d{1,2}[A-Z]{1,3}\d{4}$/i.test(candidateReg)) {
      result.registrationNumber = candidateReg;
    }
  }

  // ----- Chassis Number (improved) -----
  let chassisMatch = result.chassisNumber === "-" ? normalized.match(/Chassis\s+No\s+([A-Z0-9]+)/i) : null;                  // Page 2: "Chassis No   MAKGK773KG4100103"
  if (!chassisMatch && result.chassisNumber === "-") {
  ...
  if (chassisMatch && chassisMatch[1]) {
    const candidateChassis = chassisMatch[1].replace(/\s+/g, '').toUpperCase();
    if (!/TRAILER|REGISTRATION|MAKE|MODEL/i.test(candidateChassis)) {
      result.chassisNumber = candidateChassis;
    }
  }
...
// Generic fallback
if (!engineMatch && result.engineNumber === "-") {
  engineMatch = normalized.match(
...
if (result.seatingCapacity === "-") {
  ...
  const geoMatch = normalized.match(/Geographical\s+Area\s*(India|INDIA)\b/i) ||
                   normalized.match(/Geographical\s+Area\s*([A-Za-z\s]+?)(?=\s+Insured|\s+Policy|\s+Address|$)/i);

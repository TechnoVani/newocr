...
export default function MotorEntry() {
  ...
  const [initialDraft] = useState(() => ({ ...EMPTY_MOTOR_FORM, ...(readMotorEntryDraft() || {}) }));
  const [motorFormData, setMotorFormData] = useState(initialDraft);
  ...
  const resetPolicyContext = () => {
    clearMotorPolicyDrafts();
    setMotorFormData((current) => ({
      ...EMPTY_MOTOR_FORM,
      bqp: current.bqp,
  ...
  useEffect(() => {
    saveMotorEntryDraft(motorFormData);
  }, [motorFormData]);
  ...
  // Restore the saved hierarchy in order so every dependent select has the
  // database options it needs after a browser refresh.
  useEffect(() => {
    const restoreHierarchy = async () => {
      const saved = initialDraft;
      await loadDirectOptions("bqp", hierarchyApi.getBqp);
      if (saved.bqp) {
        await loadDirectOptions("manager", () => hierarchyApi.getReportingManagers(saved.bqp));
      }
      if (saved.bqp && saved.manager) {
        await loadDirectOptions("relationship", () =>
          hierarchyApi.getRelationshipManagers(saved.manager, saved.bqp)
        );
      }
      if (saved.bqp && saved.manager && saved.relationship) {
        await loadDirectOptions("posp", () =>
          hierarchyApi.getPosps(saved.relationship, saved.bqp, saved.manager)
        );
      }
      if (saved.posp) {
        await loadDirectOptions("reference", () => hierarchyApi.getReferences(saved.posp));
      }
    };
    restoreHierarchy();
  }, [initialDraft, loadDirectOptions]);
  ...
  const motorProps = {
    ...
    isMotorStepDisabled,
    onSubmitSuccess: resetPolicyContext,
    onNewPolicy: resetPolicyContext,
  };

...
export const validateDocuments = (data) => {
    ...
    if (hasAadhaarFront !== hasAadhaarBack) {
        const error = new Error(
            "If Aadhaar is uploaded, both front and back files are required"
        );
        error.statusCode = 400;
        throw error;
    }
    ...
    if (!hasPanCard && !(hasAadhaarFront && hasAadhaarBack)) {
        const error = new Error(
            "Upload either a PAN card or both Aadhaar front and back files"
        );
        error.statusCode = 400;
        throw error;
    }
...
class PolicyService {
    ...
    async getAllPolicies(userId) {
        return await PolicyModel.findAll(userId);
    }
    ...
    async getPolicyById(id, userId) {
        const policy = await PolicyModel.findById(id, userId);
        if (!policy) {
     ...
     */
    async searchPolicy(keyword, userId) {
        if (!keyword) {
            throw new Error("Search keyword required");
        }
        return await PolicyModel.search(keyword, userId);
    }
    ...
    async getMonthlyReport(year, month, userId) {
        ...
        const policies = await PolicyModel.findMonthlyReport(startDate, endDate, userId);
    ...
    async getRenewalReport(year, month, userId) {
        const numericYear = Number(year);
        const numericMonth = Number(month);
        ...
        const nextMonth = numericMonth === 12 ? 1 : numericMonth + 1;
        const endDate = `${nextYear}-${pad(nextMonth)}-01`;
        const policies = await PolicyModel.findRenewalsByExpiryMonth(startDate, endDate, userId);

...
function mapFrontendToBackend(frontendData) {
    ...
    data.seating_capacity = frontendData.seating_capacity || v.seatingCapacity;
...
class PolicyController {
    ...
    static async getAll(req, res, next) {
        try {
            const policies = await PolicyService.getAllPolicies(req.user.id);
    ...
    static async getByRefId(req, res, next) {
        try {
            const policy = await PolicyModel.findByRefId(req.params.refId, req.user.id);
            if (!policy) {
    ...
    static async search(req, res, next) {
        try {
            ...
            const result = await PolicyService.searchPolicy(query, req.user.id);
    ...
    static async getAllRefs(req, res, next) {
        try {
            const result = await PolicyModel.getAllReferenceIds(req.user.id);
    ...
    static async getDashboardSummary(req, res, next) {
        try {
            const result = await PolicyModel.getDashboardSummary(req.user.id);
    ...
    static async getMonthlyReport(req, res, next) {
        try {
            ...
            const result = await PolicyService.getMonthlyReport(year, month, req.user.id);
    ...
    static async getRenewalReport(req, res, next) {
        try {
            ...
            const year = req.query.year ?? now.getFullYear();
            const month = req.query.month ?? now.getMonth() + 1;
            const result = await PolicyService.getRenewalReport(year, month, req.user.id);
